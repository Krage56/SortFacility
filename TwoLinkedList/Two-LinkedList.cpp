...
void TwoLinkedList::Node::insertNext(TwoLinkedList::Node *node) {
    Node* tmp = this->next;
    this->next = node;
    node->previous = this;
    node->next = tmp;
...
void TwoLinkedList::insert(const size_t pos, TwoLinkedList::Node *node) {
    if (pos < 0) {
        throw std::out_of_range("Index of required position is "
                                "out of range\n");
    }
...
void TwoLinkedList::pushBackNode(TwoLinkedList::Node *node) {
    if (_size == 0) {
        pushFrontNode(node);
    }
    else{
...
void TwoLinkedList::pushFrontNode(TwoLinkedList::Node *node) {
    Node *tmp = _head;
    _head = node;
    _head->next = tmp;
